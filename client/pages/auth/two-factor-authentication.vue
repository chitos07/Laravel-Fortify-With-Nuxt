     ...
     <div class="flex flex-col justify-center items-center mx-auto w-3/4 min-h-screen">
               ...
               <div>
                    <div class="bg-gray-700 py-8 px-8 rounded">
                         ...
                         <div class="flex flex-col items-center space-y-4">
                              ...
                              <p class="text-white">Add additional security to your account using two factor
                                   authentication.</p>
                              <div>
          ...
          <template v-if="!twoFactorEnabled">
               <form @submit.prevent="enable">
                    <button type="submit"
                         class="bg-green-500 text-white border rounded uppercase px-2 py-1 tracking-wider leading-tight mt-4 hover:bg-green-800">enable</button>
               </form>
          </template>
          <template v-else>
               <div class="flex justify-center space-x-6 mt-4 w-3/4">
                    <button type="button"
                         class="bg-green-500 text-white border rounded uppercase px-2 py-1 tracking-wider leading-tight mt-4 hover:bg-green-800"
                         v-if="recCodes.length > 0" @click.prevent="regenerateRecoveryCodes">Regenerate Recovery
                         Codes</button>
                    <button type="button"
                         class="bg-green-500 text-white border rounded uppercase px-2 py-1 tracking-wider leading-tight mt-4 hover:bg-green-800"
                         v-if="recCodes.length === 0" @click.prevent="showRecoveryCodes">Show recovery codes</button>
                    <form @submit.prevent="disable">
                         <button type="submit"
                              class="bg-red-500 text-white border rounded uppercase px-2 py-1 tracking-wider leading-tight mt-4 hover:bg-red-800">Disable</button>
                    </form>
               </div>
          </template>

          <div class="flex justify-center items-center w-full min-h-screen absolute"
               :class="openConfirm ? 'bg-black bg-opacity-50' : ''" v-if="openConfirm">
               <div class="bg-blue-700 py-8 px-8 rounded w-2/3 h-1/4">

                    <span class="flex justify-center text-red-400 italic mb-4" v-if="errors.password">{{
                    ...
                    <form @submit.prevent="confirmPassword" class="flex flex-col h-full justify-between">
                         <div class="flex flex-col" v-if="message">
                              ...
                              <input type="password" v-model="form.password" class="mt-4 px-2 py-1 rounded"
                                   placeholder="Please confirm your password">
                         </div>

                         <div class="flex justify-between mt-4">
                              <button type="button"
                                   class="bg-red-500 text-white border rounded uppercase px-2 py-1 tracking-wider leading-tight hover:bg-red-800"
                                   @click.prevent="openConfirm = !openConfirm">Cancel</button>
                              <button type="submit"
                                   class="bg-green-500 text-white border rounded uppercase px-2 py-1 tracking-wider leading-tight hover:bg-green-800">Confirm</button>
                         </div>
                    </form>
               </div>
          </div>
