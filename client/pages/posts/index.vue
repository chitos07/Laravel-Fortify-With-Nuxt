          ...
          <main class="container mx-auto my-6 space-y-6 w-3/4">
               <div class="flex justify-between">
                    <NuxtLink to="/posts/create"
                         class="bg-blue-500 text-white border rounded uppercase px-2 py-1 tracking-wider leading-tight hover:bg-blue-800">
               ...
               <div v-for="(post, index) in posts" :key="index"
                    ...
                    <div class="flex justify-end w-1/4">
                         <NuxtLink :to="`/posts/${post.id}/edit`"
                              class="bg-green-500 text-white border rounded uppercase px-2 py-1 tracking-wider leading-tight hover:bg-green-800">
                              edit</NuxtLink>
                         <form @submit.prevent="deletePost(post.id)">
                              <button type="submit"
                                   class="bg-red-500 text-white border rounded uppercase ml-3 px-2 py-1 tracking-wider leading-tight hover:bg-red-800">delete</button>
                         </form>
                    </div>
               </div>
          </main>
